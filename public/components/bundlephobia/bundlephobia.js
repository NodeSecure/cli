...
`;
  ...
  #bunldeTask = new Task(this, {
    task: async([packageInfo]) => {
      try {
        const {
          gzip, size, dependencySizes
        } = await getJSON(`/bundle/${this.#httpName(packageInfo.name)}/${packageInfo.version}`);
  ...
  #bundlephobiaTemplate(bundle = {
    gzip: this.#DEFAULT_SIZE,
    min: this.#DEFAULT_SIZE,
    full: this.#DEFAULT_SIZE
  }) {
